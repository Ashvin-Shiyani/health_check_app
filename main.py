...
label = tk.Label(root, text="Welcome", font=("Arial", 30))  # welcome sign
label.pack(pady=10)

gender_var = tk.StringVar()  # gender button
gender_var.set("Select Gender")
dropdown = tk.OptionMenu(root, gender_var, "Male", "Female")
dropdown.configure(width=15, font=("Arial", 12))
dropdown["menu"].config(font=("Arial", 12))
dropdown.pack(anchor="w", padx=20)

weight_frame = tk.Frame(root, bg="#870c0c")  # weight entry frame
...
height_frame = tk.Frame(root, bg="#1a1a2e")  # height button
...
age_frame = tk.Frame(root, bg="#1a1a2e")  # age button
...
btn = tk.Button(root, text="Calculate", font=("Arial", 12),
                bg="#4CAF50", fg="white", command=get_inputs)
btn.pack(pady=20)
